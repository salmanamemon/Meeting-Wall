...
function stories(): array
{
	...
	if ($body === false || $status < 200 || $status >= 300)
		throw new RuntimeException($error ?: "The stories service returned HTTP $status.");
	$data = json_decode($body, true, 512, JSON_THROW_ON_ERROR);
	$rows = $data['data']['daliysaleList'] ?? $data['data']['dailySaleList'] ?? [];
	if (!is_array($rows))
		throw new RuntimeException('The stories service returned an invalid list.');
...
function leaderboardTiers(array $rankedLeaders): array
{
	...
	rsort($allCounts);
	$rankByCount = [];
	foreach ($allCounts as $position => $count)
		$rankByCount[$count] = $position + 1;
	$rankByOwner = [];
	...
	$tierLeaders = [];
	foreach ($rankedLeaders as $leader) {
		if (!in_array($leader['count'], $topThreeCounts, true))
			continue;
...
function renderSlides(array $items): string
{
	ob_start();
	include __DIR__ . '/partials/slides.php';
	return ob_get_clean();
}

// Renders the whole data-driven dashboard (#wall contents). Shared by index.php and the ?fragment=1 poll.
function renderWall(array $items, string $error = ''): string
{
	ob_start();
	include __DIR__ . '/wall.php';
	return ob_get_clean();
}

// Renders the v1 leaderboard-dashboard (#wall contents). Used by index_v1.php.
function renderWallV1(): string
{
	ob_start();
	include __DIR__ . '/wall_v1.php';
	return ob_get_clean();
}
...
// Ordinal label for a rank with the suffix superscripted, e.g. 1 -> "1<sup>st</sup>".
// Returns HTML (safe: $number is an int) — output unescaped.
function ordinal(int $number): string
{
	$suffix = ($number % 100 >= 11 && $number % 100 <= 13) ? 'th' : (['th', 'st', 'nd', 'rd'][$number % 10] ?? 'th');
...
function inTodayWindow(array $item): bool
{
	...
	if ($timestamp === null || date('Y-m-d', $timestamp) !== date('Y-m-d'))
		return false;
...
function wallData(array $items): array
{
	...
	arsort($countByOwner);
	$leaders = [];
	foreach ($countByOwner as $ownerName => $count)
		$leaders[] = ['name' => $ownerName, 'count' => $count];
...
function storiesSignature(array $items): string
{
...
function asset(string $path): string
{
	...
	return escapeHtml($path . (is_file($fullPath) ? '?v=' . filemtime($fullPath) : ''));
}
